...
function get_latest_transcation(){ 
	...
	var complete_transactions = web3.eth.getBlockTransactionCount(web3.eth.getBlock(newblock-present)['hash']) 
	console.log("Satwik Hosamani all trasncation "+complete_transactions); 
	for(var i=0;i<complete_transactions;i++){ 
		console.log(JSON.stringify(web3.eth.getTransactionFromBlock(web3.eth.getBlock(newblock-present)['hash'], i))); console.log("\n");
	}
